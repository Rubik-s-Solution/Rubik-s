...
def is_grid_3x3(centers: np.ndarray, tolerance: float=0.3) -> bool:
    ...
    # 간격 비율 확인 (너무 차이나면 격자 아님)
    if max(x_spacing) > 0:
        x_ratio = min(x_spacing) / max(x_spacing)
        if x_ratio < (1 - tolerance):
            return False
    
    if max(y_spacing) > 0:
        y_ratio = min(y_spacing) / max(y_spacing)
        if y_ratio < (1 - tolerance):
            return False
    
    return True

def find_9_stickers(bgr: np.ndarray) -> Tuple[List[cv2.RotatedRect], np.ndarray]:
    H,W = bgr.shape[:2]
    ...
    if len(rects) >= 9:
        ...
        for i in range(min(len(rects)-8, 20)):
            ...
            # 격자를 못 찾으면 중심에 가까운 9개
            rects = [rects[j] for j in sorted_idx[:9]]

    return rects[:9], mask

def warp_by_sticker_union(bgr: np.ndarray, rects: List[cv2.RotatedRect], size:int=720):
...
def tiles_labels_with_numbers(bgr: np.ndarray, margin: float=0.18):
    ...
    def patch(r,c):
        ...
        return hsv[yy0:yy1, xx0:xx1]

    def med_hsv(p):
        if p.size==0: return np.array([0,0,0],np.float32)
    ...
    for r in range(3):
        for c in range(3):
            Hm,Sm,Vm = med_hsv(patch(r,c))
            Hm,Sm,Vm = int(Hm), int(Sm), int(Vm)

            if (Sm < max(30, S20+5)) and (Vm > max(130, V60-10)):
                results.append((tile_num, "WHITE"))
                tile_num += 1
                continue

            def within(a,b,x): return (a < x <= b)
            if (Hm <= 10 or Hm >= 170) and Sm>80 and Vm>80:      lab="RED"
            elif within(10,25,Hm) and Sm>80 and Vm>80:           lab="ORANGE"
            elif within(25,42,Hm) and Sm>60 and Vm>90:           lab="YELLOW"
            elif within(42,90,Hm) and Sm>50 and Vm>80:           lab="GREEN"
            elif within(90,140,Hm) and Sm>50 and Vm>80:          lab="BLUE"
            else:
                centers = {
                    "WHITE": (0,0,235),
                    "YELLOW":(31,140,200),
                ...
                def dist(c):
                    h0,s0,v0 = centers[c]
                    dh = min(abs(Hm-h0), 180-abs(Hm-h0))
                    return (dh*2)**2 + (Sm-s0)**2 + (Vm-v0)**2
                lab = min(centers.keys(), key=dist)
            
            results.append((tile_num, lab))
            tile_num += 1
